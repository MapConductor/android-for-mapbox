package com.mapconductor.mapbox.sample
...
// Set your Mapbox public access token before the map is rendered.
// Recommended: call MapboxOptions.accessToken = "pk.eyJ1..." in your Application.onCreate().
...
private fun HomeScreen(onSelect: (Demo) -> Unit) {
    Scaffold { padding ->
        Column(
            ...
            Text("Mapbox Sample", style = MaterialTheme.typography.headlineSmall)
...
private fun MarkerDemo(modifier: Modifier = Modifier) {
    ...
    MapboxMapView(state = mapViewState, modifier = modifier.fillMaxSize()) {
        Marker(markerState)
